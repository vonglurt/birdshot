...
struct Frame {
  ...
  Gray8 full;              // optional native-resolution luma; empty = y is all there is.
                           // When present it is what the focus measure's centre crop is
                           // cut from -- the 1.x rule that a webcam should deliver the
                           // frame it actually captured.
  Rgb8 color;              // optional native-resolution colour. When present it is what
                           // gets saved; analysis still runs on the luma planes above.
